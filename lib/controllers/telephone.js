...
exports.addTelephone = function(req,res) {
  ...
  exports.updateTelephone = function(req,res) {
    var data = req.body;
      ...
      console.log(data);
    db.cyperQuery("START n=NODE(" + data.phoneId + ") SET n="+data.data,function(err,result){
        if(err)throw err;
        res.json(result.data);
    });
};
    ...
    var data = req.body;
      ...
      console.log(data);
    db.cyperQuery("START n=NODE(" + data.phoneId + ") match (n)-[r]-() delete n,r",function(err,result){
        if(err)throw err;
        res.json(result.data);
    });
};
